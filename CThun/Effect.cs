...
namespace CThun
{
    public class Effect : INotifyPropertyChanged
    {
        ...
        public Effect(string name)
        {
            Name = name;
            Count = 0;
            Log.Debug($"{LibraryInfo.Name} Effect Create {Name}: {Count}");
        }

        public int Increment(int byAmount = 1)
        {
            Log.Debug($"{LibraryInfo.Name} Effect Increment {Name}: {Count}+{byAmount}");
            Count = byAmount;
            return Count;
        }
        ...
        {
            Count = 0;
            Log.Debug($"{LibraryInfo.Name} Effect Reset {Name}: {Count}");
        }
